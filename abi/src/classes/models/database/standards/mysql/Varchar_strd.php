...
class Varchar_strd
{
	...
	private static function checkCompatibility()
    {
		if ('false' === self::$pattern->getField('bind_db')) {
			...
			if (self::$pattern->getField('length') < 1 || self::$pattern->getField('length') > 21844) {
				EventHandler::validationError("The value of 'length' property must be in the range from 1 to 21844");
			}
		}
		...
		if ('true' === self::$pattern->getField('ai')) {
		    EventHandler::validationError("The 'auto_increment' property is not supported for 'Varchar' type");
		}
        ...
        if ('' !== self::$pattern->getField('collection')) {
            EventHandler::validationError("The 'collection' property is not supported for 'Varchar' type");
        }
	}
