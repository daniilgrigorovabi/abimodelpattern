...
class Date_strd {
	...
	private static function _checkCompatibility() {
		...
		if('' !== self::$pattern->getField('length')) { EventHandler::validationError("The 'length' property is not supported for 'Date' type"); }
		if('true' === self::$pattern->getField('ai')) { EventHandler::validationError("The 'auto_increment' property is not supported for 'Date' type"); }

		if(self::$pattern->getField('default') !== '') {
			if(!preg_match($date_regexp, self::$pattern->getField('default')) &&
