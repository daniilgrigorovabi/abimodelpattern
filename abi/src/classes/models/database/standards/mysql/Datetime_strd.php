...
class Datetime_strd
{
	...
	private static function checkCompatibility()
    {
		...
		if ('' !== self::$pattern->getField('length')) {
		    EventHandler::validationError("The 'length' property is not supported for 'Datetime' type");
		}

		if ('true' === self::$pattern->getField('ai')) {
		    EventHandler::validationError("The 'auto_increment' property is not supported for 'Datetime' type");
		}

		if (self::$pattern->getField('default') !== '') {
			if (
			    !preg_match($datetime_regexp, self::$pattern->getField('default')) &&
		...
		if (self::$pattern->getField('default') !== Types::CURRENT_TIMESTAMP && 'true' === self::$pattern->getField('unique')) {
        ...
        if ('' !== self::$pattern->getField('collection')) {
            EventHandler::validationError("The 'collection' property is not supported for 'Datetime' type");
        }
	}
