...
class Mysql_Driver implements IDatabase_Driver {
	...
	private function _getDescribeTable($table_name) {
    ...
    private function _getColumnDefinition($column_params) {
        ...
        if($column_params->getField('default')) {
            if(Types::CURRENT_TIMESTAMP === $column_params->getField('default') &&
                'date' === $column_params->getField('type')) {
                goto without_default;
            }

            if(is_string($column_params->getField('default')) && Types::CURRENT_TIMESTAMP !== $column_params->getField('default')) {
	...
	public function createMPDatabase($db_name) {
		...
		if(!$sth->execute()) { EventHandler::error("Trouble with database name"); }
	}
	...
	public function createMPTable($model) {
		...
		foreach ($model as $model_name => $model_pattern) {
			...
			if($this->_getDescribeTable($model_name)) { EventHandler::error("The table with name '$model_name' already exists"); }
			...
			foreach ($model_pattern as $column_name => $column_params) {
				if('true' === $column_params->getField('bind_db')) {
                    $sql_columns_params[$column_name] .= "$column_name ".$this->_getColumnDefinition($column_params);
			...
			if($sql_columns_params) { $sql .= implode(', ', $sql_columns_params); }
	...
	public function updateMPTable($model, $old_model, $map) {
		...
		foreach ($old_model as $old_model_name => $old_model_pattern) {
			break;
		}

		foreach ($model as $model_name => $model_pattern) {
			$table_desc = $this->_getDescribeTable($old_model_name);

			if($table_desc) {
				// If need to delete table
				if (false == array_filter($map, function ($all_delete) { return $all_delete !== 'DELETE COLUMN';})) {
			...
			if ($model_name !== $old_model_name) {
				$sql_query_array[$sql_query_array_num] .= ' RENAME '.Settings::getParam('prefix').$model_name.' ';
			}
			...
			foreach ($model_pattern as $column_name => $column_params) {
				...
				if ('true' === $column_params->getField('bind_db')) {
					...
					if ('ADD COLUMN' === $old_model_field_name) {
					    $sql_query_array[$sql_query_array_num] .= " ADD $column_name ".$this->_getColumnDefinition($column_params);
						...
						$sql_query_array[$sql_query_array_num] .= $this->_getColumnDefinition($column_params);
						...
						if ($column_params->getField('unique') !== $old_column_params->getField('unique')) {
							...
							if ('false' === $column_params->getField('unique') && 'true' === $old_column_params->getField('unique')) {
								$sql_query_array[$sql_query_array_num] .= "DROP INDEX $map[$column_name]";
							}
							// From 'not unique' to 'unique'
							elseif ('true' === $column_params->getField('unique') && 'false' === $old_column_params->getField('unique')) {
								$sql_query_array[$sql_query_array_num] .= "ADD INDEX $column_name ($column_name)";
							}
						}
						// From 'unique' to 'unique' with rename column name
						elseif ($is_change_column_name && 'true' === $column_params->getField('unique') && 'true' === $old_column_params->getField('unique')) {
							$sql_query_array[$sql_query_array_num] .= "DROP INDEX $map[$column_name]";
							$sql_query_array_num++;
							$sql_query_array[$sql_query_array_num] .= "ADD INDEX $column_name ($column_name)";
						}
	...
	public function deleteMPTable($model_name) {
		if($this->_getDescribeTable($model_name)) {
