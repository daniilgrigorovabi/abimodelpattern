...
class Parser {
	...
	private static function _deleteAttrFromTag($startTag) {
	...
	private static function _retrieveTagsFromXML($msg, $flag = PREG_PATTERN_ORDER) {
		preg_match_all("|<[^\<\>]+>|U", $msg, $arrXMLTags, $flag);
		return $arrXMLTags;
	}

	private static function _getAttrValue($lastTagFullName) {
	...
	private static function _deleteBracket($tagName) {
		$prettyName = substr($tagName, 1);

		return $prettyName;
	}

	private static function _getDefaultFieldID(){
	...
	private static function _createModelField($field_params) {
		// Create an object of field
		$model_field = new ABIModel_Param();
    ...
    public static function getPath($config_path) {
        $abi_path = stristr( __DIR__, 'abi'.DIRECTORY_SEPARATOR.'src'.DIRECTORY_SEPARATOR.'classes', true);
	...
	public static function getModelPattern($model_name, $driver = false) {
		...
		if($driver) {
			$xml = file_get_contents(self::getPath('driver_patterns_rf').
									 Settings::getParam('driver').DIRECTORY_SEPARATOR.$model_name.'.xml');
		} else {
		...
		$arrXMLTags = self::_retrieveTagsFromXML($xml);
		...
		foreach($arrXMLTags[0] as $arrMsgTag) {
			$tagName = self::_deleteAttrFromTag($arrMsgTag);

			// If a start tag
			if('/' !== $tagName[1]) {
				// The start tag is waiting end tag
				if (1 === $arrTags[$tagName]) {
					$value = self::_getAttrValue($arrMsgTag);
				...
				} else if(0 === $arrTags[$tagName] && $current_model_field_name) {
					$value = self::_getAttrValue($arrMsgTag);

					$field_property_name = self::_deleteBracket($tagName);
	...
	public static function setModelPattern($model) {
		...
		if(!is_object($model)) { EventHandler::error("The model pattern must be object"); }

		foreach ($model as $model_name => $model_pattern) {

			if(!is_object($model_pattern)) { EventHandler::error("The model pattern must be object"); }
			...
			if(property_exists($model->$model_name, $model_id_name)) {
				...
				$model->$model_name->$model_id_name = self::_getDefaultFieldID();
			} else {
				$model_bind_db = false;
			}
			...
			foreach ($model_pattern as $field_key => $field_params) {
				...
				$model_field = self::_createModelField($field_params);
				...
				if(!$model_bind_db && 'true' === $model_field->getField('bind_db')) {
					...
					if('true' === $model_field->getField('bind_db')) { $count_bind_db_fields++; }
					if('true' === $model_field->getField('ai')) { $count_ai_fields++; }
			...
			if($new_xml_file = fopen($file_pattern_path, 'w')) {
				...
				if($model_bind_db) {
					try {
                        ...
                        EventHandler::error("The model '$model_name' was not created. Database connection error. ".$e->getMessage());
					}
					...
					if(!$db_inst->createMPTable($model)) {
						...
						// TODO: fatal error if unlink returned the 'false' value
						unlink(self::getPath('custom_patterns_rf').$model_name.'.xml');
						EventHandler::error("The model was not created. Cannot create the model '$model_name' to database. ".$db_inst->getLastError());
					}
				}
	...
	private static function _getFieldNameByNumber($model, $field_number_need) {
	...
	public static function updateModelPattern($model) {
		...
		foreach ($model as $model_name => $model_pattern) {
			...
			if(property_exists($model->$model_name, $model_id_name)) {
				...
				$old_id_field_name = self::_getFieldNameByNumber($old_model->$old_model_name,
				...
				if($model_bind_db) {
					if($field_key === $model_id_name) {
						...
						while ($index_new_model_field < $index_new_field) {
							$old_field_name = self::_getFieldNameByNumber($old_model->$old_model_name,
								$index_new_model_field);
			...
			if(rename($old_file_pattern_path, $old_file_pattern_path.'.OLD.xml')) {
				if($new_xml_file = fopen($file_pattern_path, 'w')) {
						...
						try {
							...
							foreach ($old_model as $old_model_name => $old_model_fields) {
								foreach ($old_model_fields as $old_model_values) {
                                    ...
                                    Validator::checkModelFieldStructure($old_model_values, $standard);
								}
							}
							...
							unlink($file_pattern_path);

							chmod($old_file_pattern_path, 0777);
							// TODO: error if unlink returned the 'false' value
							rename($old_file_pattern_path.'.OLD.xml', $old_file_pattern_path);
					...
					chmod($old_file_pattern_path.'.OLD.xml', 0777);
					// TODO: error if unlink returned the 'false' value
					unlink($old_file_pattern_path.'.OLD.xml');
	...
	public static function deleteModelPattern($model_name) {
		$db_inst = new Database();
	...
	public static function validateFrontendModelField($field) {
		...
		foreach ($field as $field_name => $field_params) {
			if(!is_object($field_params)) { EventHandler::error('The field pattern must be object'); }
